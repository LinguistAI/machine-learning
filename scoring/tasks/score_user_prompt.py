...
def find_words_in_list(message: str, word_list: list[str]):
    ...
    found_words = [
        word for word in words_in_message if word in normalized_word_list]
...
def score_user_prompt(email: str, unknownWords: list[UnknownWord], message: str):
    ...
    if not message:
        logger.error(
            f"Error while scoring user prompt. Message is required for {email}")

    if not unknownWords:
        logger.error(
            f"Error while scoring user prompt. Unknown words are required for {email}")
    ...
    unknown_words_list = [
        word.word for word in unknownWords if word.listId != None and word.isActive]
    ...
    logger.info(
        f"Unknown words in user prompt for user {email}: {unknown_words_in_sentence}")
    ...
    logger.info(
        f"Time taken to generate Gemini response for scoring user prompt {email}: {end_time - start_time}")
    ...
    for word_score in json_response:
        ...
        gramatical_correctness = int(gramatical_correctness) if int(
            gramatical_correctness) in range(1, 6) else 1
        spelling = int(spelling) if int(spelling) in range(1, 6) else 1
        punctuation = int(punctuation) if int(
            punctuation) in range(1, 6) else 1
        capitalization = int(capitalization) if int(
            capitalization) in range(1, 6) else 1
        word_choice = int(word_choice) if int(
            word_choice) in range(1, 6) else 1
        sentence_structure = int(sentence_structure) if int(
            sentence_structure) in range(1, 6) else 1
        rest_of_sentence = int(rest_of_sentence) if int(
            rest_of_sentence) in range(1, 6) else 1

        avg_score = (gramatical_correctness + spelling + punctuation +
                     capitalization + word_choice + sentence_structure + rest_of_sentence) / 7.0
        ...
        if confidence_level_change < 0:
            ...
            decrease_word_confidence(
                email, unknown_word, confidence_level_change)
        elif confidence_level_change > 0:
            confidence_level_change = abs(confidence_level_change)
            increase_word_confidence(
                email, unknown_word, confidence_level_change)
