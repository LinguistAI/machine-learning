...
urlpatterns = [
    ...
    path(f'{BASE_URL_PREFIX}/conversation/',
         include('chat.urls'), name="chat"),
    path(f'{BASE_URL_PREFIX}/mcq/', include('mcq.urls'), name="mcq"),
    path(f'{BASE_URL_PREFIX}/profile/',
         include('profiling.urls'), name="profiling"),

    # Swagger
    path(f'{BASE_URL_PREFIX}/swagger<format>/',
         schema_view.without_ui(cache_timeout=0), name='schema-json'),
    path(f'{BASE_URL_PREFIX}/swagger/', schema_view.with_ui('swagger',
         cache_timeout=0), name='schema-swagger-ui'),
    path(f'{BASE_URL_PREFIX}/redoc/', schema_view.with_ui('redoc',
