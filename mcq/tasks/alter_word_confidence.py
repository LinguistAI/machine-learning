...
def increase_word_confidence(email: str,
    ...
    if unknown_word.listId == None or unknown_word.listId == "":
        return

    previous_confidence = unknown_word.confidenceLevel
    next_confidence = previous_confidence + increase_amount

    previous_confidence_category = floor(
        previous_confidence / CONFIDENCE_LEVEL_SCALING_FACTOR)
    next_confidence_category = floor(
        next_confidence / CONFIDENCE_LEVEL_SCALING_FACTOR)
    ...
    if previous_confidence_category != next_confidence_category:
        ...
        executor.submit(request_increase_confidence_backend,
                        email, unknown_word)
...
def decrease_word_confidence(email: str,
    ...
    if previous_confidence_category != next_confidence_category:
        # Update user service to increase confidence async
        executor = ThreadPoolExecutor()
        executor.submit(request_decrease_confidence_backend,
                        email, unknown_word)
