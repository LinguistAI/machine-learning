...
def update_unknown_words(conversation_id: str, user_email: str):
    ...
    conversation_exists = Conversation.objects.filter(
        id=conversation_id).exists()

    if not conversation_exists:
        logger.error("Error while getting unknown words for {} conversation. Conversation does not exist".format(
            conversation_id))
        return None
    ...
    response = requests.post(
        USER_SERVICE_SELECT_WORD_PATH, json=request_body, headers=headers)
    ...
    for word_obj in data:
        ...
        # Check if listId is present in the word_key
        if "ownerList" in word_key:
            if "listId" in word_key.get("ownerList"):
                if word_key.get("ownerList").get("listId") is not None:
        ...
        if confidence in BACKEND_CONFIDENCE_LEVELS:
            confidence_level = max(BACKEND_CONFIDENCE_LEVELS.index(
                confidence) * CONFIDENCE_LEVEL_SCALING_FACTOR, 1)

        # Check if the word already exists in the database
        word_exists = UnknownWord.objects.filter(
            word=word, listId=listId, email=user_email).exists()

        if word_exists:
            unknown_word: UnknownWord = UnknownWord.objects.filter(
                word=word, listId=listId, email=user_email).first()
    ...
    logger.info("Unknown words for conversation {} are: {}".format(
        conversation_id, unknown_words))
