        ...
        "200": openapi.Response(
            ...
            examples={
                "application/json": {
                    ...
                    "status": 200,
                    "msg": "Conversation gathered successfully",
                    "data": {
...
def get_user_conversation_by_id(request, conversation_id: str):
    ...
    conversations = Conversation.objects.filter(userEmail=email, id=conversation_id).first()
    
    serializer = ConversationSerializer(conversations)
    
    logger.info(f"Conversation gathered successfully for user: {email} with conversation id {conversation_id}")
    
    return generate_success_response("Conversation gathered successfully", serializer.data)